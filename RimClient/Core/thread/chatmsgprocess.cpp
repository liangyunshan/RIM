...
/*!
 * @brief 添加全部类型历史消息记录查询
 * @param otherID 联系人账号id
 * @param count 需要查询的消息条数
 */
void ChatMsgProcess::appendC2CAllHistoryQueryTask(QString otherID, uint begin, uint count)
{
    TaskQueue t_newQueryTask;
    ChatInfoUnit t_unit;
    t_unit.accountId = G_User->BaseInfo().accountId;
    t_newQueryTask.otherID = otherID;
    t_newQueryTask.tskType = C2C_TASK;
    t_newQueryTask.actType = QUERY_ALLHIS;
    t_newQueryTask.msg = t_unit;
    t_newQueryTask.start = begin;
    t_newQueryTask.count = count;
    m_TaskQueue.enqueue(t_newQueryTask);
    ...
    while (runningFlag)
    {
        ...
        if(runningFlag && m_TaskQueue.size() > 0)
        {
            ...
            case QUERY_ALLHIS:
                if(m_task.tskType == C2C_TASK)
                {
                    queryC2CTaskAllHistory(m_task.otherID,m_task.start,m_task.count);
                }
                else
                {
                    queryGroupTaskAllHistory(m_task.otherID,m_task.start,m_task.count);
                }
                break;
            case QUERY_IMGHIS:
                if(m_task.tskType == C2C_TASK)
 ...
 * @param start 需要查询的消息的起始位置（最小值为0）
 * @param count 需要查询的消息条数
 * @return 查看全部类型历史消息记录操作结果
 */
bool ChatMsgProcess::queryC2CTaskAllHistory(QString otherID, uint start, uint count)
{
    allHistory.clear();
 ...
 * @param start 需要查询的消息的起始位置（最小值为0）
 * @param count 需要查询的消息条数
 * @return 查看图片类型历史消息记录操作结果
 */
bool ChatMsgProcess::queryC2CTaskImgHistory(QString otherID, uint start, uint count)
{
    //TODO LYS-20180814 实现查询单聊图片类型历史消息记录
    Q_UNUSED(otherID);
    Q_UNUSED(start);
    Q_UNUSED(count);
 ...
 * @param start 需要查询的消息的起始位置（最小值为0）
 * @param count 需要查询的消息条数
 * @return 查看文件类型历史消息记录操作结果
 */
bool ChatMsgProcess::queryC2CTaskFileHistory(QString otherID, uint start, uint count)
{
    //TODO LYS-20180814 实现查询单聊文件类型历史消息记录
    Q_UNUSED(otherID);
    Q_UNUSED(start);
    Q_UNUSED(count);
...
/*!
 * @brief ChatMsgProcess::queryGroupTaskHistory
 * @param otherID
 * @param start
 * @param count
 * @return
 */
bool ChatMsgProcess::queryGroupTaskAllHistory(QString otherID, uint start, uint count)
{
    //TODO LYS-20180814 实现查询群聊历史消息记录
    Q_UNUSED(otherID);
    Q_UNUSED(start);
    Q_UNUSED(count);
